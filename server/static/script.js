...
socket.on("ev3-message", (rawData) => {
	console.log(`Message received: ${JSON.stringify(rawData)}`);

	lastContactTimestamp = Date.now();

	const dataArr = JSON.parse(rawData.data_str);

	const data = {
		timestamp: dataArr[0],
		batteryLevel: dataArr[1],
		leftMotorSpeed: dataArr[2],
	...
	display.batteryVolts.textContent = round2(data.batteryLevel);
	display.batteryPercentage.textContent = round2(
